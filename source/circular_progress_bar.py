...
_DEFAULT_CENTER = (None, None)
_DEFAULT_FIX_CAP = False
...
class CircularProgressBar(Widget):
    ...
    def __init__(self, **kwargs):
        ...
        self._center = _DEFAULT_CENTER
        self._fix_cap = _DEFAULT_FIX_CAP
    ...
    @fix_cap.setter
    def fix_cap(self, fix_cap):
        if type(fix_cap) != bool:
            raise TypeError("Circular bar fix_cap only accepts an bool, not {}!".format(type(fix_cap)))
        else:
    ...
    def center(self, value: Iterable):
        if not isinstance(value, Iterable):
            raise TypeError("center must be iterable (e.g. list, tuple), not {}!".format(type(value)))
        else:
            self._center = value
            self._draw()
    ...
    def _draw(self):
        ...
        with self.canvas:
            ...
            if not self.center[0] == None:
                self.pos[0] = self._center[0] - self._widget_size / 2
            if not self.center[1] == None:
                self.pos[1] = self._center[1] - self._widget_size / 2
            ...
            # fix cap
            if (self.get_normalised_progress() == 0 and self._fix_cap):
                ccap = "none"
            else:
                ccap = self._cap_style
            
            # Draw the progress line
            Color(*self.progress_colour)
            Line(circle=(self.pos[0] + self._widget_size / 2, self.pos[1] + self._widget_size / 2,
                         self._widget_size / 2 - self._thickness, 0, self.get_normalised_progress() * 360),
                 width=self._thickness, cap=ccap, cap_precision=self._cap_precision)
